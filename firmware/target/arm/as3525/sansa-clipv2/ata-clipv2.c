...
/* Driver for the SD controller inside AS3531 SoC */
...
static void mci_set_clock_divider(int divider)
{
    (void)divider;
#if 0
    int clock = MCI_CLOCK;
...
//    wakeup_signal(&transfer_completion_signal);
    ...
    switch(status)
    {
        ...
        case 0x820:     /* ? 1 time while manual copy from FIFO */
...
static int sd_init_card(void)
{
    ...
    if( (card_info.csd[0]>>30) == 0)
    {
        /* CSD version 1.0 */
        c_size = ((card_info.csd[1] & 0x3ff) << 2) + (card_info.csd[2]>>30) + 1;
        c_mult = 4 << ((card_info.csd[2] >> 15) & 7);
        card_info.max_read_bl_len = 1 << ((card_info.csd[1] >> 16) & 15);
        card_info.block_size = BLOCK_SIZE;     /* Always use 512 byte blocks */
        card_info.numblocks = c_size * c_mult * (card_info.max_read_bl_len/512);
        card_info.capacity = card_info.numblocks * card_info.block_size;
    }
    else if( (card_info.csd[0]>>30) == 1)
    {
        /* CSD version 2.0 */
        c_size = ((card_info.csd[1] & 0x3f) << 16) + (card_info.csd[2]>>16) + 1;
        card_info.max_read_bl_len = 1 << ((card_info.csd[1] >> 16) & 0xf);
        card_info.block_size = BLOCK_SIZE;     /* Always use 512 byte blocks */
        card_info.numblocks = c_size << 10;
        card_info.capacity = card_info.numblocks * card_info.block_size;
    }
    ...
    if(!send_cmd(SD_SET_BLOCKLEN, card_info.block_size, MCI_NO_RESP,
                 NULL))
        return -12;

    card_info.initialized = 1;

    mci_set_clock_divider(1); /* full speed */
...
static void init_controller(void)
{
    ...
    /* set MCLK divider */
    mci_set_clock_divider(CLK_DIV(AS3525_PCLK_FREQ, AS3525_SD_IDENT_FREQ));
}
...
{
    info->sector_size=card_info.block_size;
...
static int sd_transfer_sectors(unsigned long start, int count, void* buf, bool write)
{
    ...
    else
    {
        ...
        if(card_info.ocr & (1<<30) ) /* SDHC */
            ...
            ret = send_cmd(cmd, start * BLOCK_SIZE,
                    MCI_NO_RESP, NULL);
...
{
#if defined(BOOTLOADER) || defined(SANSA_CLIPV2) /* we don't need write support in bootloader */
    (void) start;
    (void) count;
    (void) buf;
    return -1;
